...
import (
	...
	"github.com/bisoncorps/saido/charts"
	"github.com/bisoncorps/saido/cmd"
...
func main() {
	...
	if err := termdash.Run(ctx, t, c, termdash.KeyboardSubscriber(quitter), termdash.RedrawInterval(charts.RedrawInterval)); err != nil {
		panic(err)
	}
}
...
func newRollText(ctx context.Context) (*text.Text, error) {
	...
	i := 0
	go charts.Periodic(ctx, 1*time.Second, func() error {
...
func newSparkLines(ctx context.Context) (*sparkline.SparkLine, *sparkline.SparkLine, error) {
	...
	const max = 100
	go charts.Periodic(ctx, 250*time.Millisecond, func() error {
	...
	go charts.Periodic(ctx, 500*time.Millisecond, func() error {
		v := int(rand.Int31n(max + 1))
...
func newGauge(ctx context.Context) (*gauge.Gauge, error) {
	...
	go charts.Periodic(ctx, 2*time.Second, func() error {
...
func newHeartbeat(ctx context.Context) (*linechart.LineChart, error) {
	...
	step := 0
	go charts.Periodic(ctx, charts.RedrawInterval/3, func() error {
